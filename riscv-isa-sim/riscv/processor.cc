...
void processor_t::disasm(insn_t insn)
{
  ...
  if (last_pc != state.pc || last_bits != bits) {
    ...
    fprintf(stderr, "core %3d: Mode: %s, Domain: %s %s : 0x%016" PRIx64 " (0x%08" PRIx64 ") %s\n",
            id, pro_mode[state.prv], state.sec_level ? "Trust" : "Normal", tag_table[mmu->get_tag()->load_tag(state.pc)],
            state.pc, bits, disassembler->disassemble(insn).c_str());
    last_pc = state.pc;
    last_bits = bits;
    executions = 1;
  } else {
