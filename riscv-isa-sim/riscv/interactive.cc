...
{
  char* pro_mode[4] = {"PRV_U", "PRV_S", "PRV_H", "PRV_M"};
  processor_t *p = get_core(args[0]);
  fprintf(stderr, "Mode: %s Domain: %s 0x%016" PRIx64 "\n", 
                   pro_mode[p->state.prv],
                   p->state.sec_level ? "Trust" : "Normal", 
                   get_pc(args));
}
...
reg_t sim_t::get_mem(const std::vector<std::string>& args)
{
  ...
  {
    case 0:
      val = mmu->loads_uint64(addr);
      break;
    case 4:
      val = mmu->loads_uint32(addr);
      break;
    case 2:
    case 6:
      val = mmu->loads_uint16(addr);
      break;
    default:
      val = mmu->loads_uint8(addr);
      break;
  }
  return val;
}

reg_t sim_t::get_tag(const std::vector<std::string>& args)
{
  if(args.size() != 1 && args.size() != 2)
    throw trap_interactive();
  ...
  val = mmu->get_tag()->load_tag(addr-(addr%8));
  return val;
}
...
{
  char* tag_table[4] = {" N", "TC", "TU", "TS"};
  fprintf(stderr, "%s : 0x%016" PRIx64 "\n", tag_table[get_tag(args)], get_mem(args));
}
