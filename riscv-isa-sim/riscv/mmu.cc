...
void mmu_t::set_mtstatus(uintptr_t status) {
...
tlb_entry_t mmu_t::fetch_slow_path(reg_t vaddr)
{
  ...
  debug_warn ("vaddr %p, paddr %p\n",vaddr, paddr);
  /* Check tags and do optional modeswitch */
  /// zheligaile
  if (!tag->tagcheck(vaddr, FETCH))
...
void mmu_t::load_slow_path(reg_t addr, reg_t len, uint8_t* bytes, bool tagcheck)
{
  ...
  /// zheligaile
  if (tagcheck && !tag->tagcheck(addr, LOAD))
