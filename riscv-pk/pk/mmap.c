...
static pte_t* __walk_internal(uintptr_t addr, int create)
{
  ...
  for (int i = (VA_BITS - RISCV_PGSHIFT) / RISCV_PGLEVEL_BITS - 1; i > 0; i--) {
    ...
    // printk("addr %p level %d, idx %x =%d=\n",addr, i, idx, (!(t[idx] & PTE_V)) );
...
int __valid_user_range(uintptr_t vaddr, size_t len)
{
  if (vaddr + len < vaddr)
    return 0;
  if (vaddr == 0x80000000) // TODO：add a csr
    return 1;
...
static int __handle_page_fault(uintptr_t vaddr, int prot)
{
  ...
  if (pte == 0 || *pte == 0 || !__valid_user_range(vaddr, 1)) {
    printk("%x Not valid user range\n", vaddr);
    return -1;
  }
    
  else if (!(*pte & PTE_V))
  {
    //printk("Create looks like addr %p pte %x *pte %x\n", vaddr, pte, *pte);
  ...
  //pte_t perms;
  //if (vaddr == 0x80000000) 
  //  perms = pte_create(0, prot_to_type(prot, 0));
  ...
  //printk("prot %x \n", prot);
 // printk("pte %x *pte %x\n", pte, *pte);
 // printk("perms %x  ", perms);
 // printk("(*pte & perms) %x  \n", (*pte & perms));
  if ((*pte & perms) != perms)
  {
   printk("kernel to user *pte = %lx perms = %lx \n", *pte, perms);
   // if () return 0;
    return -1;
  }
...
void __map_kernel_range(uintptr_t vaddr, uintptr_t paddr, size_t len, int prot)
{
  ...
  for (uintptr_t a = vaddr, i = 0; i < n; i++, a += RISCV_PGSIZE)
  {
    pte_t* pte = __walk_create(a);
    kassert(pte);
    if ((paddr == 0x80000000) && (i == 0))
      *pte = pte_create((a + offset) >> RISCV_PGSHIFT, prot_to_type(prot, 1));
    else 
...
uintptr_t pk_vm_init()
{
 ...
 // printk ("end %p\n", &_end);
  ...
  //printk ("first_free_paddr %lx = \nfirst_free_page %lx + \nfree_pages %lx * RISCV_PGSIZE %lx\n", 
  //         first_free_paddr,      first_free_page,          free_pages      ,RISCV_PGSIZE );
