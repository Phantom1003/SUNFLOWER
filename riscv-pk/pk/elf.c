...
extern uintptr_t _test_start[];
extern uintptr_t _test_end[];
  extern uintptr_t _trust_supervisor_function_start[];
  extern uintptr_t _trust_supervisor_function_end[];
...
{
  printk("\033[1;32m[Kernel] Load user program: %s\033[0m\n", fn);
  ...
  //printk("%p %p\n", _trust_supervisor_function_start, _trust_supervisor_function_end);
  file_decref(file);
  return;
